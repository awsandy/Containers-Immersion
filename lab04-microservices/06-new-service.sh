...
tdarn=$(aws ecs list-task-definitions --query taskDefinitionArns | jq -r .[] | grep Monolith-Definition-containersid | tail -1)
export TF_VAR_lb=$(aws elbv2 describe-load-balancers --query LoadBalancers[].DNSName | jq -r .[])
ldarn=$(aws ecs list-task-definitions --query taskDefinitionArns | jq -r .[] | grep containersid-like | tail -1)
